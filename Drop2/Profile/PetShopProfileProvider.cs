...
namespace PetShop.Profile
{
    public sealed class PetShopProfileProvider : ProfileProvider {
		...
		private static readonly IPetShopProfileProvider dal = new ServiceProfileDAL.PetShopProfileProvider();
        ...
        {
            //NOTE: Basic rule: If it's authenticated, then we use the FirebaseGuid for ID, otherwise we use the annonymous Guid (UserName) generated by ASP.NET
			string username = (string)context["UserName"];
			bool isAuthenticated = (bool)context["IsAuthenticated"];

            string userId;
            //TODO: Temp solution, the issue at hand is during Profile_MigrateAnonymous event
            //      the IsAuthenticated will be true but we still need to access cart items from the anonymous profile which will be a GUID 
            //      generated by ASP.NET
            if (isAuthenticated && username.Length < 30)
            {
            ...
            foreach (SettingsProperty prop in collection) {
				...
				switch(pv.Property.Name) {
					case PROFILE_SHOPPINGCART:
                        pv.PropertyValue = GetCartItems(userId, true);
						break;
					case PROFILE_WISHLIST:
                        pv.PropertyValue = GetCartItems(userId, false);
                        break;
					case PROFILE_ACCOUNT:
						if(isAuthenticated)
							pv.PropertyValue = GetAccountInfo(userId);
						break;
				}
		...
		public override void SetPropertyValues(SettingsContext context, SettingsPropertyValueCollection collection) 
        {
            ...
            int uniqueID = dal.GetUniqueID(userId, isAuthenticated, false, ApplicationName);
			...
			UpdateActivityDates(userId, false);
		}
		...
		private static Cart GetCartItems(string username, bool isShoppingCart) {
            ...
            var cart = dal.GetCartItems(username, applicationName, isShoppingCart);

            foreach (CartItemInfo cartItem in cart) {
		...
		// Verifies user name for size and comma
