...
namespace ProductServiceCore.Conrollers
{
    ...
    {
        private const string SQL_SELECT_ITEMS_BY_PRODUCT = "SELECT Item.ItemId AS id, Item.Name, Inventory.Qty AS quantity, Item.ListPrice AS price, Product.Name as productName, Item.Image, Product.CategoryId, Product.ProductId FROM \"MSPETSHOP4\".Item INNER JOIN" +
        ...
        private const string SQL_SELECT_ITEMS_BY_ID = "SELECT Item.ItemId AS id, Item.Name, Item.ListPrice as price, Product.Name AS ProductName, Item.Image, Product.CategoryId, Product.ProductId FROM \"MSPETSHOP4\".Item " +
                                                      "INNER JOIN \"MSPETSHOP4\".Product ON Item.ProductId = Product.ProductId WHERE Item.ItemId = :itemId";
        
        private ConnectionSettings ConfigSettings { get; set; }
