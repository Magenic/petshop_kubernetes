...
function getWidgetUrl() {
  return '/widget.html#recaptcha=' + getRecaptchaMode();
}
...
var handleSignedInUser = function(user) {
  ...
  window.location.replace("/login_fba.aspx?uid=" + user.uid + "&displayname=" + user.displayName + ((returnurl == null) ? "" : "&ReturnUrl=" + returnurl) );
};
...
firebase.auth().onAuthStateChanged(function(user) {
  ...
  if (user) {
      //window.location.replace("/default2.aspx?uid=" + user.uid + "&displayname=" + user.displayName);
      // User is signed in.
